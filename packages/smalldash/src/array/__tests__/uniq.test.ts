...
test('uniq array regardless of order', () => {
  ...
  transformed
    .sort()
    .map((num, index) => expect(num).toBe(expected.sort()[index]));
});
...
test('uniq array of objects by nested property', () => {
  ...
  transformed.map((obj, index) =>
    expect(obj.name.id).toBe(expected[index].name.id)
  );
});
...
test('uniq preserves order by having choosing first uniq options', () => {
  ...
  transformed.map((obj, index) =>
    expect(obj.name.other).toBe(expected[index].name.other)
  );
});
